...
<template>
	<div class="drop-input">
		...
		<el-popover ref="popover1" placement="top-start" width="210" trigger="focus">
			<ul>
				<li :class="activeIndex==index?'active-li':''" v-for="(item,index) in tempOptions" @click="clickOption(item.option,index)">{{item.id+' | '+item.option}}</li>
			</ul>
		</el-popover>
	</div>
</template>
		...
		methods:{
			...
			inputEnter(){
				if(this.activeIndex!=null){
					this.inputVal=this.tempOptions[this.activeIndex].option;
					this.$refs.popover1.showPopper=false;
				}
			},
			//activeIndex值增加
			activeIndexAdd(){
				if(this.activeIndex==null || this.activeIndex>this.tempOptions.length-2){
					this.activeIndex=0;
				}else{
					this.activeIndex++;
				}
			...
			activeIndexReduce(){
				...
				if(this.activeIndex<0){
					this.activeIndex=this.tempOptions.length-1;
				}
			},
			//在输入框进行输入时，进行各种判断
			inputCheckVal(){
			...
			//使用正则表达式对options数据进行过滤，并存储到newOptions数组中
			regexFilter(){
				for(let i=0;i<this.options.length;i++){
					let optionStr=JSON.stringify(this.options[i]);
					if(this.regex.test(optionStr)){
		...
		watch:{
			tempOptions:function () {
				if(this.tempOptions.length==0){
					this.$refs.popover1.showPopper=false
				}else if(this.tempOptions.length==1){
					this.activeIndex=0;
				}
			},
		},
